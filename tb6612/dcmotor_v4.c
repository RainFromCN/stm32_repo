...
void DCMOTOR_UpdateEx(DCMOTOR_TypeDef* M, uint32_t len) {
    ...
    M[0].ENC.tick_fresh = 1;
}

...用户修改（2）//滴答更新，放入1ms的定时器中断中
/* 将单个对象进行滴答更新 */
...
...用户修改（3）//放入while中轮询，或者紧接着滴答更新
/* 将多个对象进行轮询更新 */
void DCMOTOR_CallbackEx(DCMOTOR_TypeDef* M, uint32_t len) {

    if (M[0].ENC.tick_fresh == 0) return;
    M[0].ENC.tick_fresh = 0;
...
...用户修改（4）//放入while中轮询，或者直接紧接着滴答更新
/* 将单个对象进行轮询更新 */
void DCMOTOR_Callback(DCMOTOR_TypeDef* M) {

    if ((*M).ENC.tick_fresh == 0) return;
    (*M).ENC.tick_fresh = 0;
